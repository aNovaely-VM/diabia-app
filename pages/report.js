// pages/report.js — Extraction texte PDF côté client (plus de limite de taille)
...
export default function Report() {
  ...
  // Extraire le texte du PDF côté client
  const extractTextFromPDF = async (arrayBuffer) => {
    ...
    for (let i = 1; i <= numPages; i++) {
      setLoadingStep(`Lecture page ${i}/${numPages}...`);
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      // Extraire le texte côté client (pas de limite de taille!)
      setLoadingStep('Extraction du texte...');
      const pdfText = await extractTextFromPDF(arrayBuffer);
      
      if (pdfText.trim().length < 100) {
        throw new Error('Impossible d\'extraire le texte du PDF. Le fichier est peut-être une image scannée.');
      }

      setLoadingStep('Analyse en cours...');
      await analyzeReport(pdfText, f.name);
    } catch (err) {
  ...
  const analyzeReport = async (pdfText, filename) => {
    try {
      // Envoyer uniquement le texte, pas le PDF complet!
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {loading && (
          <div className="fade-up" style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <p style={{ color: '#374151', fontSize: 12, marginTop: 6 }}>Extraction locale, pas de limite de taille</p>
          </div>
        )}
          ...
          <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ background: '#0f1117', border: '1px solid #1a1f2e', borderRadius: 16, padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                <div>
                  <p style={{ color: '#475569', fontSize: 12, marginBottom: 4 }}>Periode analysee</p>
            ...
            {activeTab === 'summary' && result.summary && (
              <div style={{ background: '#0f1117', border: '1px solid #1a1f2e', borderRadius: 16, padding: '16px' }}>
                ...
                <StatRow label="Glycemie moyenne" value={result.summary.avg_glucose} unit=" mg/dL" good={v=>v<=154} warn={v=>v<=183} />
                ...
                {result.key_findings?.length > 0 && (
                  <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #1a1f2e' }}>
                    <p style={{ color: '#475569', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 10 }}>Observations cles</p>
                  ...
                  <div key={i} style={{ background: '#0f1117', border: '1px solid #1a1f2e', borderRadius: 14, padding: '14px' }}>
                    ...
                    {(rec.current_value || rec.suggested_value) && (
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        ...
                        {rec.suggested_value && <span style={{ background: 'rgba(59,130,246,0.12)', color: '#60a5fa', fontSize: 12, padding: '4px 10px', borderRadius: 8 }}>Suggere : {rec.suggested_value}</span>}
                      </div>
                    )}
                  </div>
                ))}
            ...
            {activeTab === 'patterns' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {result.hyperglycemia_patterns?.length > 0 && (
                  <>
                    <p style={{ color: '#475569', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Tendances hyperglycemiques</p>
                    {result.hyperglycemia_patterns.map((p, i) => (
                      <div key={i} style={{ background: '#0f1117', border: '1px solid #1a1f2e', borderRadius: 14, padding: '14px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                          <p style={{ color: '#e2e8f0', fontSize: 15, fontWeight: 600 }}>{p.time_slot}</p>
                          {p.occurrences && <span style={{ color: '#f87171', fontSize: 13 }}>{p.occurrences} fois</span>}
                        </div>
                ...
                {!result.hyperglycemia_patterns?.length && !result.sensor_analysis && !result.smartguard_analysis && (
                  <div style={{ background: '#0f1117', border: '1px solid #1a1f2e', borderRadius: 14, padding: '20px', textAlign: 'center' }}>
                    <p style={{ color: '#64748b', fontSize: 14 }}>Aucune tendance specifique detectee</p>
                  </div>
                )}
              </div>
            )}
